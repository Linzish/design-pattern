...
public class Calculator {
    ...
    // 构造函数传参，并解析
    public Calculator(String expStr) {  // expStr = 算数表达式
        // 安排运算先后顺序
        // 使用栈
        ...
        for (int i = 0; i < charArray.length; i++) {
            switch (charArray[i]) {
                ...
                case '*':
                case '/':
                    throw new UnsupportedOperationException("暂不支持乘除法");
                default:
                    //如果是一个 Var（数字，值） 就创建要给 VarExpression 对象，并 push 到 stack
                    stack.push(new VarExpression(String.valueOf(charArray[i])));
                    break;
            }
        }
        //当遍历完整个 charArray 数组（算数表达式）后，stack 就得到最后 Expression
